...
function setChooseTableStyle(the,tmp)
{
  ...
  for(var i=0;i<the.rows.length;i++)
  {
		...
		if(the.rows[i].type!="-3")
		{
			...
			if(tmp==0)
			  the.rows[i].onclick=chooseRow;//选择行
		}

		if(the.rows[i].rowIndex == 0 && the.rows[i].type=="-3" ){
			for(k=0;k<the.rows[i].cells.length;k++){
				with(the.rows[i].cells[k]){
					onclick = clicktd;
				}
			}
		}
...
function chooseRow()
{
  ...
  var thetby = this.parentElement;
  var thetbl = thetby.parentElement;
  if(this.choose!="choosed")
  {		 
		if(this.type=="0")
		{
			  ...
			  {
					thetbl.rows[i].bgColor=thetbl.rows[i].bgSourceColor;
					thetbl.rows[i].choose="";
					thetbl.rows[i].style.color="#000000";
					if(thetbl.rows[i].type=="1") 
					{
						thetbl.rows[i].cells[0].innerHTML="&nbsp;&nbsp;" //修改3.2
					}
					chooseNum = 0;
			  }
		}		
...
function setNoChoose(the){
  	//var the = document.getElementById(tablename);
	  for(var i=0;i<the.rows.length;i++)
	  {
			the.rows[i].bgColor=the.rows[i].bgSourceColor;
			the.rows[i].choose="";
			the.rows[i].style.color="#000000";

			chooseNum = 0;
	  }	
}

//Add in order to sort by Ysy
...
function sort_tab(the_tab,col,mode,flag){
	...
	for(i=j;i<the_tab.rows.length;i++){
...
function clicktd(){
	...
	var thettr= this.parentElement;
	cur_col = this.cellIndex;
	var thetby = thettr.parentElement;
  	var thetbl = thetby.parentElement;
	
	if(this.mode==undefined) this.mode = false;  
	this.mode = !this.mode;
	
/*	if(sort_col!=null){
		with(thetbl.rows[0].cells[sort_col])
			removeChild(lastChild);
	}
	with(thetbl.rows[0].cells[cur_col])
		appendChild(this.mode?arrowUp:arrowDown);
*/
	act_fc_old = thetbl.rows[0].cells[cur_col].runtimeStyle.color;
	if(sort_col!=null){
		with(thetbl.rows[0].cells[sort_col])
			runtimeStyle.color=act_fc_old;
	}
	...
	if(thetbl.type == '1'){
		var the_name = thetbl.id;			
		var obj = eval("parent."+secframe+"."+sectable);
		sort_tab(obj,cur_col,this.mode,"1");
	}else{
		sort_tab(thetbl,cur_col,this.mode,"0");
	}
	sort_col=cur_col;
