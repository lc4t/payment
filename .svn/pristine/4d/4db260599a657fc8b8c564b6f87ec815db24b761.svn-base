...
public class WeixinServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_WEIXIN);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = WeixinCharge.getTransactionId(vo, body, request.getRemoteAddr());
		}
		else if (stype.equals("2"))
		{
			ret = WeixinCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("weixin model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("weixin order id->(" + uid + ")" + ret);
