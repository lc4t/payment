...
public class PengyouwanServlet extends HttpServlet {
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (stype.equals("1")) {
			...
			vo.setPayType(Constants.PAY_TYPE_PENGYOUWAN);
			vo.setCallbackUrl(cburl);
			vo.setCurrency(Constants.CURRENCY_RMB);
			vo.setUnit(Constants.CURRENCY_UNIT_YUAN);

			ret = OrderUtil.getTransactionId(vo, Constants.PAY_TYPE_PENGYOUWAN,
					"");
		} else if (stype.equals("2")) {
			OrderStatusVO st = new OrderStatusVO();
			ret = OrderUtil.checkOrdersStatus(payIds, st);
		} else {
			System.out.println("PengyouwanServlet model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("PengyouwanServlet order id->(" + uid + ")" + ret);
