...
import noumena.payment.amazon.AmazonCharge;
...
public class AmazonServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//支付用到的参数
		String receipt = request.getParameter("receipt");		//支付收据
		String amazonid = request.getParameter("amazonid");		//支付uid
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_AMAZON);
			vo.setCallbackUrl(cburl);
			
			vo.setExInfo(receipt);
			vo.setPayId(amazonid);
			
			ret = AmazonCharge.getTransactionId(vo);
		}
		else if (stype.equals("3"))
		{
			...
			System.out.println("amazon charge ->" + uid + "(" + System.currentTimeMillis() + ")");
			ret = AmazonCharge.getTransactionIdAndStatus(vo);
		}
		else
		{
			System.out.println("amazon check order ids->" + payIds);
			ret = AmazonCharge.checkOrdersStatus(payIds);
		}

		System.out.println("amazon order id->(" + (uid == null ? "-" : uid) + ")" + ret);
