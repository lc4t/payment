package noumena.payment.dao.servlet.sina;
...
public class SinaServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_SINA);
			vo.setCallbackUrl(cburl);
			
			ret = SinaCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = SinaCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(sina model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(sina order id)->(" + uid + ")" + ret);
