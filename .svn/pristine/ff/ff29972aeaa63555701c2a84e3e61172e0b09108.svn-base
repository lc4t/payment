...
public class PPServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			if(!"com.kongzhong.nuomena.ios.d1cn.pp".equals(pkgid)){
				...
				vo.setExInfo("");
				vo.setPayType(Constants.PAY_TYPE_PP);
				vo.setCallbackUrl(cburl);
				
				ret = PPCharge.getTransactionId(vo);
			}
		...
		else if (stype.equals("verify"))
		{
			...
			ret = ChannelVerify.verify("cnpp", "", token, true);
		}
		else
		{
			ret = PPCharge.checkOrdersStatus(payIds);
		}

		System.out.println("pp order id->(" + uid + ")" + ret);
