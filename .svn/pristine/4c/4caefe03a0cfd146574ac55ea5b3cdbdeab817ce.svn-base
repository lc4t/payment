...
public class TencentYCharge {
	...
	public static int purchase(Orders order, TencentYOrderVO ordervo) {
		...
		// ")=====channel(tencent_purchase_params)->" + sb.toString());
		...
		try {
			...
			minwen += "&"
					+ URLEncoder
							.encode("/v3/r" + TencentYParams.PURCHASE_URI,
									"utf-8").replace("*", "%2A")
							.replace("+", "%20");
			minwen += "&"
					+ URLEncoder.encode(paramstr, "utf-8").replace("*", "%2A")
							.replace("+", "%20");
			// System.out.println("=====(" + DateUtil.getCurTimeStr() +
			// ")=====channel(tencent_purchase_minwen)->" + minwen);

			byte[] sigbyte = TencentYSignUtils.HmacSHA1Encrypt(minwen,
					encryptKey);
			BASE64Encoder base64 = new BASE64Encoder();
			String sig = base64.encode(sigbyte);
			sig = URLEncoder.encode(sig, "utf-8").replace("*", "%2A")
					.replace("+", "%20");

			String cookiestr = "";
			cookiestr += "session_id=" + URLEncoder.encode(session_id, "utf-8");
			cookiestr += "; session_type="
					+ URLEncoder.encode(session_type, "utf-8");
			cookiestr += "; org_loc="
					+ URLEncoder.encode(TencentYParams.PURCHASE_URI, "utf-8");
			...
			System.out.println("=====(" + DateUtil.getCurTimeStr()
					+ ")=====channel(tencent order(" + order.getOrderId()
					+ ") purchase urlstr)->" + urlstr);
			...
			System.out.println("=====(" + DateUtil.getCurTimeStr()
					+ ")=====channel(tencent_order(" + order.getOrderId()
					+ ")_purchase_ret)->" + res);
			...
			if (json.getInt("ret") == 0 && balance >= 0) {
				ret = 1;
				bean.updateOrderAmountPayIdExinfo(order.getOrderId(),
						json.getString("billno"), amt,
						exinfo + "#" + json.getString("balance"));
	...
	public static void clearPurchase(Orders order, TencentYOrderVO ordervo,
			int balance) {
		try {
			String appid = ordervo.getAppid();
			long ts = Calendar.getInstance().getTimeInMillis() / 1000;
	...
	public static int getBalance(Orders order, TencentYOrderVO ordervo) {
		...
		try {
			...
			String encryptKey = params.getParams(ordervo.getAppid())
					.getAppkey() + "&";
			...
			minwen += "&"
					+ URLEncoder
							.encode("/v3/r" + TencentYParams.GETBALANCE_URI,
									"utf-8").replace("*", "%2A")
							.replace("+", "%20");
			minwen += "&"
					+ URLEncoder.encode(paramstr, "utf-8").replace("*", "%2A")
			...
			cookiestr += "; session_type="
					+ URLEncoder.encode(session_type, "utf-8");
			cookiestr += "; org_loc="
					+ URLEncoder.encode(TencentYParams.GETBALANCE_URI, "utf-8");
			...
			System.out.println("tencent_order(" + order.getOrderId()
					+ ")_getBalance_urlstr-->" + urlstr);
	...
	public static void main(String args[]) {
		 ...
		 * vo.setOpenid("opnFdwH0N-656_LHMtiuVjaX3jxg"); vo.setOpenkey(
		 * "OezXcEiiBSKSxW0eoylIeF3fsmX53crxnEk9mfKu22Q81FpmbrPjle0QBxtYREdVE0-M3MJa9w2KKC2SYYnKEsutcj3okmGJ3FikSZb4Iudv_R1W4lqlBlWPsprJKAeBPOp6G-yAQVEwOjjlQd7VMg"
		 * ); vo.setPf("myapp_m_wx-00000000-android-00000000-ysdk");
		 * vo.setPfkey("6ea4619b5560a159ecdd609989b86666"); vo.setType("");
		 * vo.setZoneid("3999"); getBalance(vo);
		 */
		String a = URLDecoder
