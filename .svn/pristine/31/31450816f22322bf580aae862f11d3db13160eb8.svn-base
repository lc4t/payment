package noumena.payment.dao.servlet.anqu;
...
public class AnquServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_ANQU);
			vo.setCallbackUrl(cburl);
			
			ret = AnquCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = AnquCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("anqu model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("anqu order id->(" + uid + ")" + ret);
