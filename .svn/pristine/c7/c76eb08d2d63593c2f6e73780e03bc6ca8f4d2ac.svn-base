package noumena.payment.dao.servlet.apus;
...
public class ApusServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_APUS);
			vo.setCallbackUrl(cburl);
			
			ret = ApusCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = ApusCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("Apus model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("Apus order id->(" + uid + ")" + ret);
