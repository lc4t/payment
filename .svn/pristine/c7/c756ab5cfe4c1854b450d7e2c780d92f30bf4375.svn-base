package noumena.payment.dao.servlet.xyzs;
...
public class XyzsServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_XYZS);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = XyzsCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = XyzsCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("xyzs model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("xyzs order id->(" + uid + ")" + ret);
