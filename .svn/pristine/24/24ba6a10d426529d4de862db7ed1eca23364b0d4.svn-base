...
public class WandoujiaServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (stype.equals("1")) {
			...
			vo.setExInfo("");
			vo.setPayType(Constants.PAY_TYPE_WANDOUJIA);
			vo.setCallbackUrl(cburl);

			ret = WandoujiaCharge.getTransactionId(vo);
		} else if (stype.equals("verify")) {
			String appid = request.getParameter("appid"); 
			String token = request.getParameter("sid"); // sid
			ret = WandoujiaCharge.getAccountId(token, appid);
		} else if ("sign".equals(stype)) {
			Map<String, String> map = OrderUtil.getParamMapFromHttp(request);
			try {
				ret = WandoujiaCharge.getSign(map);
			} catch (Exception e) {
				e.printStackTrace();
			...
			ret = WandoujiaCharge.checkOrdersStatus(payIds);
		}

		System.out.println("wandoujia order id->(" + uid + ")" + ret);
