...
public class DownjoyServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String appid = request.getParameter("appid");			//当乐分配的appid
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_DOWNJOY);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			ret = DownjoyCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = DownjoyCharge.checkOrdersStatus(payIds);
		}
		else
		{
		...
		System.out.println("downjoy order id->" + ret);
