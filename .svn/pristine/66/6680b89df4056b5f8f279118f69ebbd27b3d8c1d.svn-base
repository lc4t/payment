...
public class BaiduServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setExInfo(appid);
			vo.setPayType(Constants.PAY_TYPE_BAIDU);
			vo.setCallbackUrl(cburl);
			
			ret = BaiduCharge.getTransactionId(vo);
		}
		else if (stype.equals("verify"))
		{
			String token = request.getParameter("token");
			token = URLDecoder.decode(token, "utf-8");
			ret = ChannelVerify.verify("cnduokoo", "1452", token, true);
		}
		else
		{
			ret = BaiduCharge.checkOrdersStatus(payIds);
		}

		System.out.println("baidu order id->(" + uid + ")" + ret);
