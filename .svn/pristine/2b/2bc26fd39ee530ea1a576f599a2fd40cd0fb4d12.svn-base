package noumena.payment.dao.servlet.aiyingyong;
...
import noumena.payment.c185sy.C185syCharge;
import noumena.payment.getorders.OrderUtil;
import noumena.payment.model.Orders;
import noumena.payment.util.Constants;
import noumena.payment.util.DateUtil;

public class AiYingYongServlet extends HttpServlet {
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (stype.equals("1")) {
			...
			vo.setPayType(Constants.PAY_TYPE_AIYINGYONG);
			vo.setCallbackUrl(cburl);
			vo.setCurrency(Constants.CURRENCY_RMB);
			vo.setUnit(Constants.CURRENCY_UNIT_YUAN);

			ret = OrderUtil.getTransactionId(vo, Constants.PAY_TYPE_AIYINGYONG,
					"");
		} else if (stype.equals("2")) {
			ret = C185syCharge.checkOrdersStatus(payIds);
		} else if (stype.equals("3")) {
			OrdersBean ordersBean=new OrdersBean();
			Orders orders=ordersBean.qureyOrder("20170321180359578L6i");
			SendOrderInfo.sendOrderInfo(orders);

		} else {
			System.out.println("AiYingYongServlet model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("AiYingYongServlet order id->(" + uid + ")" + ret);
