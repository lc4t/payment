package noumena.payment.dao.servlet.gionee;
...
public class GioneeServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_GIONEE);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = GioneeCharge.getTransactionId(vo,itemname);
		}
		else if (stype.equals("2"))
		{
			ret = GioneeCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("gionee model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("gionee order id->(" + uid + ")" + ret);
