...
public class TCC2Servlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setExInfo(subid + "#" + transid + "#" + productid);
			vo.setPayType(Constants.PAY_TYPE_TCC2);
			vo.setCallbackUrl(cburl);
			vo.setSubId(subid);
			
			ret = TCC2Charge.getTransactionId(vo);
		}
		else
		{
			ret = TCC2Charge.checkOrdersStatus(payIds);
		}

		System.out.println("tcc2 order id->" + ret);
