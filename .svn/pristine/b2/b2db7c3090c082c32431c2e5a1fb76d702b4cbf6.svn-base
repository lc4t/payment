...
public class ZhaorongServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setSign(appid);
			vo.setPayType(Constants.PAY_TYPE_ZHAORONG);
			vo.setCallbackUrl(cburl);
			
			ret = ZhaorongCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = ZhaorongCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("zhaorong model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("zhaorong order id->(" + uid + ")" + ret);
