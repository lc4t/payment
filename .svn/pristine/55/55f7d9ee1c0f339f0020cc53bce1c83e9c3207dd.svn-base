...
public class MeizuCharge
{
	private static MeizuParams params = new MeizuParams();
	...
	public static String getCallbackFromMeizu(Map<String,String> meizuparams)
	{
    	...
    	JSONObject json = JSONObject.fromObject(meizuparams);
    	MeizuOrderVO ordervo = (MeizuOrderVO)JSONObject.toBean(json,MeizuOrderVO.class);
		...
		else
		{
			// 服务器签名验证失败
	    	ret = String.format(str, "0","失败");
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(meizu cb)->(appid=" + ordervo.getGid()+",content="+ minwen +",sign="+ miwen);
		}
    	
		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(meizu cb ret)->" + ret);
		
		String path = OSUtil.getRootPath() + "../../logs/meizucb/" + DateUtil.getCurTimeStr().substring(0, 8);
	...
	{
		params.initParams(MeizuParams.CHANNEL_ID, new MeizuParamsVO());
