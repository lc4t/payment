...
import noumena.payment.tcc.TCCCharge;
...
public class TCCServlet extends HttpServlet
{
	 ...
	 * 		1 - 客户端请求支付
	 * 		2 - 客户端请求验证订单
	 * 		11 - TCC请求Token服务成功状态回调
	 * 		12 - TCC请求Token服务取消状态回调
	 * 		13 - TCC请求Token服务失败状态回调
	 * 
	 * @param request
	 *            the request send by the client to the server
	 * @param response
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		{
			payTypeId = Constants.PAY_TYPE_TCC;
		}
		...
		if (stype.equals("1"))
		{
			...
			ret = TCCCharge.getTransactionId(vo);
		}
		else
		{
			ret = TCCCharge.checkOrdersStatus(payIds);
		}
