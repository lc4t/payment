package noumena.payment.dao.servlet.vivonew;
...
public class VivoNewServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_VIVONEW);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = VivoNewCharge.getTransactionId(vo, orderTitle, orderDesc);
		}
		else if (stype.equals("2"))
		{
			ret = VivoNewCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("vivonew model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("vivonew order id->(" + uid + ")" + ret);
