...
public class UnionpayServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_UNIONPAY);
			vo.setCallbackUrl(cburl);
			
			ret = UnionpayCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = UnionpayCharge.checkOrdersStatus(payIds);
		}
		else if (stype.equals("query"))
		{
			String querydate = request.getParameter("querydate");
			String queryorderid = request.getParameter("queryorderid");
			ret = UnionpayCharge.queryOrderFromUnionpay(querydate, queryorderid);
		}
		else
		{
			System.out.println("unionpay model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("unionpay order id->(" + uid + ")" + ret);
