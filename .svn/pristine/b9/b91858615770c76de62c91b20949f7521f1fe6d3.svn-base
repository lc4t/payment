...
public class LGUplusServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//支付方的参数
//		String pid = request.getParameter("pid");				//已购买的部分商品ID 值
		String pid = itemid;
		String appid = request.getParameter("appid");			//部分货币化母商品ID
		String ctn = request.getParameter("ctn");				//用户终端编号
		String ukey = request.getParameter("ukey");				//获取的 Unique-key 
		String txid = request.getParameter("txid");				//获取的 txid
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_LGUPLUS);
			vo.setCallbackUrl(cburl);
			
			ret = LGUplusCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
//			ret = LGUplusCharge.checkOrdersStatus(payIds);
		}
...
//			vo.setSign(ukey);
			...
			ret = LGUplusCharge.getTransactionIdAndStatus(vo, pid, appid, ctn, mac, ukey);
		}
		else
		{
			System.out.println("lgu+ model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("lgu+ order id->(" + uid + ")" + ret);
