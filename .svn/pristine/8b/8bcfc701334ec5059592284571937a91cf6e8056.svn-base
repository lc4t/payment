package noumena.payment.dao.servlet.yijie;
...
public class YijieServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//支付方的参数
		String sdk = request.getParameter("sdk");//渠道在易接服务器的 ID
		//验证订单用参数
		...
		if (stype.equals("1"))
		{
			...
			String paytype = "yj_"+sdk.toLowerCase();
			...
			vo.setExInfo(Constants.PAY_TYPE_YIJIE);
			
			ret = YijieCharge.getTransactionId(vo,paytype);
		}
		else if (stype.equals("2"))
		{
			ret = YijieCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(yijie model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(yijie order id)->(" + uid + ")" + ret);
