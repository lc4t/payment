package noumena.payment.dao.servlet.coolpad;
...
public class CoolpadServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String appid = request.getParameter("appid");		//酷派分配的游戏的appkey
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_COOLPAD);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = CoolpadCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = CoolpadCharge.checkOrdersStatus(payIds);
		}
		else if (stype.equals("info"))
		{
			//通过code得到支付信息（uid和token）
			ret = CoolpadCharge.getTransactionInfo(code, appid);
		}
		else
		{
			System.out.println("coolpad model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("coolpad order id->(" + uid + ")" + ret);
