...
public class GoogleYuanServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_GOOGLE);
			vo.setCallbackUrl(cburl);
			
			ret = GoogleCharge.getTransactionId(vo);
		}
//		else if (stype.equals("2"))
//		{
//			ret = GoogleCharge.checkOrdersStatus(payIds);
//		}
		else if (stype.equals("receipt")) 
		{
			ret = GoogleCharge.purchase(signature, signedData);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay order id)->(" + uid + ")" + ret);
