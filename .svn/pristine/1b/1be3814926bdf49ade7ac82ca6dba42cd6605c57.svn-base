...
public class IOSServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 支付用到的参数
		String receipt = request.getParameter("receipt"); // 苹果支付收据
		String orderid = request.getParameter("orderid"); // 苹果支付收据
		...
		if (stype.equals("1")) {
			if (!"com.kongzhong.nuomena.ios.d1cn".equals(pkgid)) {
				...
				if ("21".equals(flag)) {
					vo.setPayType(Constants.PAY_TYPE_IOS21);
				} else {
					vo.setPayType(Constants.PAY_TYPE_IOS);
				}
				vo.setCallbackUrl(cburl);

				vo.setExInfo(receipt);

				ret = IOSCharge.getTransactionId(vo, flag);
			}
			...
			System.out.println("ios charge ->" + uid + "("
					+ System.currentTimeMillis() + ")");
			ret = IOSCharge.getTransactionIdAndStatus(vo, flag);
		} else if (stype.equals("receipt")) {
			// 验证收据是否合法
			System.out.println("ios charge new ->" + uid + "("
					+ System.currentTimeMillis() + ") orderid:" + orderid);
			ret = IOSCharge.verifyReceipt(orderid, receipt, flag);
		} else if (stype.equals("2")) {
			System.out.println("ios check order ids->("
					+ (uid == null ? "-" : uid) + ")" + payIds);
			ret = IOSCharge.checkOrdersStatus(payIds, flag);
		} else {
			ret = "model is invalid";
		}

		System.out.println("ios order id->" + ret);
