...
public class TongbuServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setSign(appid);
			vo.setPayType(Constants.PAY_TYPE_TONGBU);
			vo.setCallbackUrl(cburl);
			
			ret = TongbuCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = TongbuCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("tongbu model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("tongbu order id->(" + uid + ")" + ret);
