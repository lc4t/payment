...
public class RibaoServlet extends HttpServlet{
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_RIBAO);
			vo.setSign(appid);
			
			ret = RibaoCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = RibaoCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(ribao model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(ribao order id)->(" + uid + ")" + ret);
