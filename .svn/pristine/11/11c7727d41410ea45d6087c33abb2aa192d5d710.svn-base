package noumena.payment.dao.servlet.linyou;
...
public class LinyouServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_LINYOU);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = LinyouCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = LinyouCharge.checkOrdersStatus(payIds);
		}else if(stype.equals("token")){
			ret = LinyouCharge.getUidByToken(token,appid);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(linyou model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(linyou order id)->(" + uid + ")" + ret);
