package noumena.payment.dao.servlet.qianbao;
...
public class QianbaoServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_QIANBAO);
			vo.setCallbackUrl(cburl);
			
			ret = QianbaoCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = QianbaoCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("qianbao model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("qianbao order id->(" + uid + ")" + ret);
