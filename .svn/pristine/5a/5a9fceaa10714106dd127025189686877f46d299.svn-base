package noumena.payment.dao.servlet.omg;
...
public class OMGServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_OMG);
			vo.setCallbackUrl(cburl);
			
			ret = OMGCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = OMGCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("OMG model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("OMG order id->(" + uid + ")" + ret);
