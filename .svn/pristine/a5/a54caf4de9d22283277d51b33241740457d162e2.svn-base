...
public class UCServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String appid = request.getParameter("appid"); // UC分配的appid
		...
		if (stype == null) {
			...
			if (!"com.kongzhong.nproject.uc".equals(pkgid)) {

			}
			if (payprice == null || payprice.equals("")) {
				payprice = "0.00";
			} else {
				payprice = new DecimalFormat("0.00")
						.format(new Float(payprice));
			}
			...
			vo.setPayType(Constants.PAY_TYPE_UC);
			vo.setCallbackUrl(cburl);
			vo.setExInfo(appid);
			ret = UCCharge.getTransactionId(vo);
		} else if (stype.equals("5")) {
			String token = request.getParameter("sid"); // sid
			ret = UCCharge.getAccountId(token, appid);
		} else if ("sign".equals(stype)) {
			Map<String, String> map = OrderUtil.getParamMapFromHttp(request);
			try {
				ret = UCCharge.getSign(map, request);
			} catch (Exception e) {
				e.printStackTrace();
				ret = "get sign error" + e.toString();
			}
		} else {
			ret = UCCharge.checkOrdersStatus(payIds);
		}

		System.out.println("uc order id ret->" + ret);
