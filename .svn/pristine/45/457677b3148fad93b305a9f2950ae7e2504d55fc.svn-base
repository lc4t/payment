package noumena.payment.dao.servlet.baiduyun;
...
public class BaiduyunServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_BAIDUYUN);
			vo.setCallbackUrl(cburl);
			
			ret = BaiduyunCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = BaiduyunCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("baiduyun model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("baiduyun order id->(" + uid + ")" + ret);
