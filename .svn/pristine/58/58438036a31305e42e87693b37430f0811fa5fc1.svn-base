package noumena.payment.dao.servlet.vivo;
...
public class VivoServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			if(!"com.kongzhong.publish.gz.vivo".equals(pkgid)){
				...
				vo.setPayType(Constants.PAY_TYPE_VIVO);
				vo.setCallbackUrl(cburl);
				vo.setSign(appid);
				
				ret = VivoCharge.getTransactionId(vo, orderTitle, orderDesc);
			}
			
		}
		else if (stype.equals("2"))
		{
			ret = VivoCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("vivo model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("vivo order id->(" + uid + ")" + ret);
