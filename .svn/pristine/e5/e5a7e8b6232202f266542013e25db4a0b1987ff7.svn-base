...
import noumena.payment.wo.WoCharge;
...
public class woServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_WO);
			vo.setCallbackUrl(cburl);
			
			ret = WoCharge.getTransactionId(vo);
		}
		else
		{
			System.out.println("wo check order ids->" + payIds);
			ret = WoCharge.checkOrdersStatus(payIds);
		}

		System.out.println("wo order id->" + ret);
