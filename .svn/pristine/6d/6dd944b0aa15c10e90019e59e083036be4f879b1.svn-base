package noumena.payment.dao.servlet.renren;
...
public class RenrenServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_RENREN);
			vo.setCallbackUrl(cburl);
			
			ret = RenrenCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = RenrenCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(renren model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(renren order id)->(" + uid + ")" + ret);
