package noumena.payment.dao.servlet.pptv;
...
public class PPTVServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_PPTV);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = PPTVCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = PPTVCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("pptv model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("pptv order id->(" + uid + ")" + ret);
