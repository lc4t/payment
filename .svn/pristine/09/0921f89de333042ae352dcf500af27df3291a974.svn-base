package noumena.payment.dao.servlet.naver;
...
public class NaverServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//支付方的参数
		String paymentSeq = request.getParameter("orderid");	//
		String appid = request.getParameter("appid");			//
		String naverversion = request.getParameter("naverversion"); //自定义Naver版本号，用于区分不同参数
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_NAVER);
			vo.setCallbackUrl(cburl);
			
			ret = NaverCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = NaverCharge.checkOrdersStatus(payIds);
		}
		else if (stype.equals("3"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_NAVER);
			vo.setCallbackUrl(cburl);
			vo.setPayId(paymentSeq);
			vo.setSign(appid);
			vo.setExInfo(naverversion);
			ret = NaverCharge.getTransactionIdAndStatus(vo);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(naver model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(naver order id)->(" + uid + ")" + ret);
