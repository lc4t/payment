package noumena.payment.dao.servlet.kunlun;
...
import noumena.payment.kunlun.KunlunCharge;
import noumena.payment.model.Orders;
import noumena.payment.util.Constants;
import noumena.payment.util.DateUtil;

public class KunlunServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setSign(appid);
			vo.setPayType(Constants.PAY_TYPE_KUNLUN);
			vo.setCallbackUrl(cburl);
			
			ret = KunlunCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = KunlunCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("kunlun model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("kunlun order id->(" + uid + ")" + ret);
