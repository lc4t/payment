...
public class NowServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (stype.equals("1")) {
			...
			vo.setPayType(Constants.PAY_TYPE_NOW);
			vo.setCallbackUrl(cburl);

			ret = NowCharge.getTransactionId(vo);
		} else if (stype.equals("2")) {
			ret = NowCharge.checkOrdersStatus(payIds);
		} else if (stype.equals("3")) {
			ret = NowCharge.verifySign(request);
		}else if (stype.equals("4")) {
			String orderId = request.getParameter("orderId"); 
			ret = NowCharge.getPayStatus(orderId);
		}  
		else {
			System.out.println("xianzai model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("xianzai order id->(" + uid + ")" + ret);
