package noumena.payment.dao.servlet.itools;
...
import noumena.payment.itools.CiToolsCharge;
import noumena.payment.model.Orders;
import noumena.payment.util.Constants;
import noumena.payment.util.DateUtil;

public class CiToolsServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//支付方的参数
		String appid = request.getParameter("appid");			//分配的游戏id
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_ITOOLS);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			
			ret = CiToolsCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = CiToolsCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("itools model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("itools order id->(" + uid + ")" + ret);
