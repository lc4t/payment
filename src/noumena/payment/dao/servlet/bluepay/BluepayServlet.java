package noumena.payment.dao.servlet.bluepay;
...
public class BluepayServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_BLUEPAY);
			vo.setCallbackUrl(cburl);
			vo.setExInfo(currency);
			
			ret = BluepayCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = BluepayCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(bluepay model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(bluepay order id)->(" + uid + ")" + ret);
