...
public class HuaweiServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			if(!"com.kongzhong.publish.gz.huawei".equals(pkgid)){
				...
				vo.setPayType(Constants.PAY_TYPE_HUAWEI);
				vo.setCallbackUrl(cburl);
				vo.setSign(appid);
				
				ret = HuaweiCharge.getTransactionId(vo, orderTitle, orderDesc);
			}
			
		}
		else if (stype.equals("2"))
		{
			ret = HuaweiCharge.checkOrdersStatus(payIds);
		}
		else if (stype.equals("params"))
		{
			//获取支付私钥和悬浮密钥
			ret = HuaweiCharge.getParamsjson(appid);
		}
		else
		{
			System.out.println("huawei model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("huawei order id->(" + uid + ")" + ret);
