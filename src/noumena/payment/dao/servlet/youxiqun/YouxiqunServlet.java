package noumena.payment.dao.servlet.youxiqun;
...
public class YouxiqunServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//支付方的参数
		String code = request.getParameter("code"); //通过sdk产生的authorization_code
		String appid = request.getParameter("client_id"); //游戏群授权的ClientID
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_YOUXIQUN);
			vo.setCallbackUrl(cburl);
			vo.setSign(appid);
			vo.setExInfo(access_token);
			
			ret = YouxiqunCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = YouxiqunCharge.checkOrdersStatus(payIds);
		}
		else if (stype.equals("info"))
		{
			//用code换取access_token
			ret = YouxiqunCharge.getAccessToken(appid, code);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(youxiqun model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(youxiqun order id)->(" + uid + ")" + ret);
