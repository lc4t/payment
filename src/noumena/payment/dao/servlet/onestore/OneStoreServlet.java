package noumena.payment.dao.servlet.onestore;
...
import noumena.payment.onestore.OneStoreCharge;
import noumena.payment.util.Constants;
import noumena.payment.util.DateUtil;

public class OneStoreServlet extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_ONESTORE);
			vo.setCallbackUrl(cburl);
			vo.setExInfo(currency);

			ret = OneStoreCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = OneStoreCharge.checkOrdersStatus(payIds);
		}
		else if (stype.equals("3"))
		{
			ret = OneStoreCharge.verifyReceipt(payId,txid, appid, currency);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr()
					+ ")=====channel(onestore model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr()
				+ ")=====channel(onestore order id)->(" + uid + ")" + ret);
