package noumena.payment.dao.servlet.iapppay;
...
public class IapppayServlet extends HttpServlet {
	 ...
	 * post.
	 * 
	 * model: 1 - 客户端请求支付 2 - 客户端请求验证订单 11
	 * 
	 * @param request
	 *            the request send by the client to the server
	 * @param response
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 支付方的参数
		String appid = request.getParameter("appid"); // 起点分配的游戏id
		...
		if (stype.equals("1")) {
			...
			vo.setPayType(Constants.PAY_TYPE_IAPPAY);
			vo.setCallbackUrl(cburl);
			vo.setItemId(itemid);
			System.out
					.println("Iapppay Orders param==============================>"
							+ vo);
			ret = OrderUtil.getTransactionId(vo, Constants.PAY_TYPE_IAPPAY, "");
		} else if (stype.equals("2")) {
			OrderStatusVO st = new OrderStatusVO();
			ret = OrderUtil.checkOrdersStatus(payIds, st);
		} else {
			System.out
					.println("Iapppay model invalid-==============================>"
							+ stype);
			ret = "invalid";
		}

		System.out
				.println("Iapppay order id-====================================>("
						+ uid + ")" + ret);
