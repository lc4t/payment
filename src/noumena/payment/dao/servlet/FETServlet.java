...
public class FETServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//支付方的参数
		String productname = request.getParameter("name");		//支付页面显示商品名称用
		String appname = request.getParameter("appname");		//支付页面显示商城名称用
		...
		if (stype.equals("1"))
		{
			...
			vo.setExInfo(productname + "#" + appname);
			vo.setProductId(productname);
			vo.setPayType(Constants.PAY_TYPE_FET);
			vo.setCallbackUrl(cburl);
			
			ret = FETCharge.getTransactionId(vo);
		}
		else
		{
			ret = FETCharge.checkOrdersStatus(payIds);
		}

		System.out.println("fet order id->" + ret);
