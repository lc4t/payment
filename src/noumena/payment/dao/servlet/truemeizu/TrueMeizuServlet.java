package noumena.payment.dao.servlet.truemeizu;
...
public class TrueMeizuServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String product_subject = request.getParameter("product_subject");//订单标题
		String buy_amount = request.getParameter("buy_amount");//购买数量
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_MEIZU);
			vo.setCallbackUrl(cburl);
			vo.setExInfo(product_subject);
			vo.setSign(appid);
			vo.setItemNum(Integer.parseInt(buy_amount));
			
			ret = TrueMeizuCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			ret = TrueMeizuCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(meizu model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(meizu order id)->(" + uid + ")" + ret);
