package noumena.payment.dao.servlet.tag;
...
//		//支付用到的参数
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_TAG);
			vo.setCallbackUrl(cburl);
			vo.setExInfo(prid);
			
//			ret = TAGCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
//			ret = TAGCharge.checkOrdersStatus(payIds);
		...
		else if(stype.equals("3"))
		{
			...
			ret = TAGCharge.getTransactionIdAndStatus(vo,prid);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(TAG model invalid)->" + stype);
			ret = "invalid";
		}

		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(TAG order id)->(" + uid + ")" + ret);
