...
public class GnetopServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (stype.equals("1")) {
			...
			vo.setPayType(Constants.PAY_TYPE_GNOTOP);
			vo.setCallbackUrl(cburl);

			ret = GnetopCharge.getTransactionId(vo);
		} else if (stype.equals("2")) {
			ret = GnetopCharge.checkOrdersStatus(payIds);
		} else {
			System.out
					.println("Gnetop model invalid------------------------------------------------->"
							+ stype);
			ret = "invalid";
		}

		System.out.println("Gnetop order id->(" + uid + ")" + ret);
