...
public class MOLServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (stype.equals("1"))
		{
			...
			vo.setAppId(pkgid);
			String paytype = Constants.PAY_TYPE_MOL_POINT;
			if (payprice == null)
			{
				payprice = "0";
				paytype = Constants.PAY_TYPE_MOL_CARD;
			}
...
//			vo.setExInfo(currencyCode);
			
			ret = MOLCharge.getTransactionId(vo, paytype);
		}
		else if (stype.equals("2"))
		{
			ret = MOLCharge.checkOrdersStatus(payIds);
		}
		else
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(mol model invalid)->" + stype);
			ret = "invalid";
		}
		
		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(mol order id)->(" + uid + ")" + ret);
