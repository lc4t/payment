package noumena.payment.dao.servlet.yinni;
...
import noumena.payment.yinni.YinniOrderVO;

public class YinniServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		//验证订单用参数
		String order_id = request.getParameter("order_id");			//待查询的所有订单号，以“,”分隔
		...
		if (stype.equals("1"))
		{
			...
			vo.setPayType(Constants.PAY_TYPE_YINNI);
			vo.setCallbackUrl(cburl);
			
			ret = YinniCharge.getTransactionId(vo);
		}
		else if (stype.equals("2"))
		{
			YinniOrderVO yovo = new YinniOrderVO();
			yovo.setOrder_id(order_id);
			yovo.setPay_id(pay_id);
			yovo.setPrice(payprice);
			YinniCharge.getCallbackFromyinni(yovo);
			ret = YinniCharge.checkOrdersStatus(order_id);
		}
		else
		{
			System.out.println("yinni model invalid->" + stype);
			ret = "invalid";
		}

		System.out.println("yinni order id->(" + uid + ")" + ret);
