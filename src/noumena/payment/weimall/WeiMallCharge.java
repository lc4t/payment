package noumena.payment.weimall;
...
public class WeiMallCharge
{
	public static String getTransactionId(Orders order)
	{
		...
		else
		{
			if (cburl.indexOf("?") == -1)
			{
				cburl += "?pt=" + Constants.PAY_TYPE_WEIMALL + "&num=1";
			}
			else
			{
				cburl += "&pt=" + Constants.PAY_TYPE_WEIMALL + "&num=1";
	...
	public static String checkUid(String gameid, String sid, String uid) throws Exception
	{
		//0、验证请求是否合法
		//1、通过sid（淘宝id）找到对应的游戏、服务器、验证地址等信息
		//2、去游戏验证uid是否合法

		WeiMaillUserInfoVO vo = new WeiMaillUserInfoVO();
		vo.setUid(uid);
		
		//1
		PayServer server = getGameServerInfo(sid);
		if (server == null)
		{
			//sid不合法
			vo.setStatus("-2");
	...
	public static String getCallbackWeiMall(String gameid, String sid, String uid, String uniqueuid, String itemid, String amount, int num, String sign, String orderid)
	{
		//0、根据淘宝订单号判断是否是重复订单
		//1、验证请求是否合法
...
//		}
		...
		try
		{
			...
			Parameter += "&pt=" + Constants.PAY_TYPE_WEIMALL;
			Parameter += "&num=" + num;
			cburl += "?" + Parameter;
			...
			order.setPayType(Constants.PAY_TYPE_WEIMALL);
	...
	{
		//1、验证请求是否合法
		String minwen = signstr + WeiMallParams.PRIVATE_KEY;
	...
	private static PayServer getGameServerInfo(String sid)
	{
		//2、通过sid（淘宝id）找到对应的游戏、服务器、回调地址等信息
		PayServerBean payServerBean = new PayServerBean();
		return payServerBean.get(sid);
	}

	public static String getServerInfo(String gameid)
	{
		//得到区服信息
		WeiMaillServerInfoVO vo = new WeiMaillServerInfoVO();
		List<WeiMaillServerInfoVO> ret = new ArrayList<WeiMaillServerInfoVO>();
		
		PayServer server = null;
		PayServerBean payServerBean = new PayServerBean();
		List<PayServer> payServer =null;
		if (gameid == null || gameid.equals("0") || gameid.equals(""))
		{
			payServer = payServerBean.select();
		}
		else
		{
			payServer = payServerBean.selectByGame(gameid);
		}
		
		for (int i = 0 ; i < payServer.size() ; i++)
		{
			server = payServer.get(i);
			
			vo = new WeiMaillServerInfoVO();
			vo.setGameid(server.getGameId());
		...
		JSONArray arr = JSONArray.fromObject(ret);
		return arr.toString();
	}
}
