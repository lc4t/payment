package noumena.payment.google;

import java.security.KeyFactory;
import java.security.PublicKey;
import java.security.Signature;
import java.security.spec.X509EncodedKeySpec;
import java.util.ArrayList;
import java.util.List;

import net.sf.json.JSONArray;
import net.sf.json.JSONObject;
import noumena.payment.bean.OrdersBean;
import noumena.payment.model.Orders;
import noumena.payment.util.Constants;
import noumena.payment.util.DateUtil;
import noumena.payment.util.NotifyPurchase;
import noumena.payment.vo.OrderIdVO;
import noumena.payment.vo.OrderStatusVO;

public class GoogleYuanCharge
{
	private static GoogleParams params = new GoogleParams();
	private static boolean testmode = false;
	
	public static boolean isTestmode()
	{
		return testmode;
	}
	public static void setTestmode(boolean testmode)
	{
		GoogleYuanCharge.testmode = testmode;
	}
	
	public static String getTransactionId(Orders order)
	{
		order.setCurrency(Constants.CURRENCY_DIVERSE);
		order.setUnit(Constants.CURRENCY_UNIT_FEN);
		
		OrdersBean bean = new OrdersBean();
		String cburl = order.getCallbackUrl();
		String payId;
		if (cburl == null || cburl.equals(""))
		{
			payId = bean.CreateOrder(order);
		}
		else
		{
			if (cburl.indexOf("?") == -1)
			{
				cburl += "?pt=" + Constants.PAY_TYPE_GOOGLE;
			}
			else
			{
				cburl += "&pt=" + Constants.PAY_TYPE_GOOGLE;
			}
			cburl += "&currency=" + Constants.CURRENCY_DIVERSE;
			cburl += "&unit=" + Constants.CURRENCY_UNIT_FEN;
			
			payId = bean.CreateOrder(order, cburl);
		}
		order.setCallbackUrl(cburl);
		String date = DateUtil.formatDate(order.getCreateTime());
		OrderIdVO orderIdVO = new OrderIdVO(payId, date);
		
		JSONObject json = JSONObject.fromObject(orderIdVO);
		return json.toString();
	}
	
//	public static String checkOrdersStatus(String payIds)
//	{
//		String[] orderIds = payIds.split(",");
//
//		OrdersBean bean = new OrdersBean();
//		List<Orders> orders = bean.qureyOrders(orderIds);
//		List<OrderStatusVO> statusret = new ArrayList<OrderStatusVO>();
//		for (int i = 0 ; i < orders.size() ; i++)
//		{
//			Orders order = orders.get(i);
//			int status = order.getKStatus();
//			OrderStatusVO st = new OrderStatusVO();
//			st.setPayId(order.getOrderId());
//			if (status == Constants.K_STSTUS_DEFAULT || status == Constants.K_CON_ERROR)
//			{
//				//如果订单状态是初始订单或者是网络连接有问题状态，返回不知道
//				st.setStatus(3);
//			}
//			else if (status == Constants.K_STSTUS_SUCCESS)
//			{
//				//如果订单已经成功，直接返回订单状态
//				st.setStatus(1);
//			}
//			else
//			{
//				//订单已经失败，直接返回订单状态
//				st.setStatus(2);
//			}
//			statusret.add(st);
//		}
//		JSONArray arr = JSONArray.fromObject(statusret);
//		
//		return arr.toString();
//	}

	public static String purchase(String signature, String signedData)
	{
		System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay purchase)-> (signature:"+signature+",signedData:"+signedData+")");

		return verifyPurchaseToList(signedData, signature);
	}

	public static String verifyPurchaseToList(String signedData, String signature)
	{
		List<OrderStatusVO> statusret = new ArrayList<OrderStatusVO>();
		String ret = JSONArray.fromObject(statusret).toString();
		
		if (signedData == null)
		{
			System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay)-> signedData is null");
			return ret;
		}
		
		JSONObject jObject;
		String appId = "";
		try
		{
			jObject = JSONObject.fromObject(signedData);
			appId = jObject.getString("packageName");
		}
		catch (Exception e)
		{
			e.printStackTrace();
			return ret;
		}
		
		//1、验证签名是否正确
		boolean verified = false;
		try
		{
			if (!signature.isEmpty())
			{
				String base64EncodedPublicKey = params.getParams(appId).getAppkey();
				if (base64EncodedPublicKey == null)
				{
					base64EncodedPublicKey = params.getParams("default").getAppkey();
				}
				PublicKey key = generatePublicKey(base64EncodedPublicKey);
				if (key == null)
				{
					System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay)-> key is null");
					return ret;
				}
				verified = verify(key, signedData, signature);
				if (!verified)
				{
					base64EncodedPublicKey = params.getParams("GP.base64EncodedPublicKey.com.noumena.android.k6tw").getAppkey();
					key = generatePublicKey(base64EncodedPublicKey);
					verified = verify(key, signedData, signature);
					if (!verified)
					{
						System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay)-> verified is false");
						return ret;
					}
				}
			}
			else
			{
				System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay)-> signature is null");
				return ret;
			}
		}
		catch (Exception e)
		{
			e.printStackTrace();
			return ret;
		}		
		
		//2、解析传输数据
		try
		{
			OrderStatusVO st = new OrderStatusVO();
			
			int purchaseState = jObject.getInt("purchaseState");
			String productId = jObject.getString("productId");
			String packageName = jObject.getString("packageName");
			long purchaseTime = jObject.getLong("purchaseTime");
			String orderId = jObject.optString("orderId", "");
			String notifyId = null;

			if (jObject.has("notificationId")) 
			{
				notifyId = jObject.getString("notificationId");
			}
			String developerPayload = jObject.optString("developerPayload",null);
			JSONObject json = JSONObject.fromObject(developerPayload);
			String payId = json.getString("orderid");
			st.setPayId(payId);

			OrdersBean bean = new OrdersBean();
			Orders order = bean.qureyOrder(payId);

			if (order == null)
			{
				System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay)-> order is null");
				st.setStatus(3);
				statusret.add(st);
				JSONArray arr = JSONArray.fromObject(statusret);
				return arr.toString();
			}
			else if (order.getKStatus()==Constants.K_STSTUS_SUCCESS) 
			{
				System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay)-> order is existing");
				st.setStatus(1);
				statusret.add(st);
				JSONArray arr = JSONArray.fromObject(statusret);
				return arr.toString();
			}
			else
			{
				bean.updateOrderAmountPayIdExinfo(payId, orderId, order.getAmount()+ "", signedData);
			}
			
			String itemId = order.getItemId();
			int pos = productId.lastIndexOf(".");
			if (pos >= 0) {
				productId = productId.substring(pos + 1);
			}
			pos = itemId.lastIndexOf(".");
			if (pos >= 0) {
				itemId = itemId.substring(pos + 1);
			}

			if (purchaseState != 0) 
			{
				st.setStatus(2);
				bean.updateOrderKStatus(payId, Constants.K_STSTUS_ERROR);
			}
			else if (!itemId.equals(productId))
			{
				st.setStatus(2);
				bean.updateOrderKStatus(payId, Constants.K_STSTUS_ERROR);
			} 
			else
			{
				bean.updateOrderKStatus(payId, Constants.K_STSTUS_SUCCESS);
				st.setStatus(1);
				
				//通知kakao成功的订单
				NotifyPurchase.notify(payId);
			}
			statusret.add(st);
		}
		catch (Exception e)
		{
			e.printStackTrace();
		}
		JSONArray arr = JSONArray.fromObject(statusret);
		return arr.toString();
	}

	
	/**
	 * Generates a PublicKey instance from a string containing the
	 * Base64-encoded public key.
	 * 
	 * @param encodedPublicKey
	 *            Base64-encoded public key
	 * @throws IllegalArgumentException
	 *             if encodedPublicKey is invalid
	 */
	public static PublicKey generatePublicKey(String encodedPublicKey) throws Exception
	{
		byte[] decodedKey = Base64.decode(encodedPublicKey);
		KeyFactory keyFactory = KeyFactory.getInstance("RSA");
		return keyFactory.generatePublic(new X509EncodedKeySpec(decodedKey));
	}

	/**
	 * Verifies that the signature from the server matches the computed
	 * signature on the data. Returns true if the data is correctly signed.
	 * 
	 * @param publicKey
	 *            public key associated with the developer account
	 * @param signedData
	 *            signed data from server
	 * @param signature
	 *            server signature
	 * @return true if the data and signature match
	 */
	public static boolean verify(PublicKey publicKey, String signedData, String signature)
	{
		Signature sig;
		try
		{
			sig = Signature.getInstance(GoogleParams.SIGNATURE_ALGORITHM);
			sig.initVerify(publicKey);
			sig.update(signedData.getBytes());
			if (!sig.verify(Base64.decode(signature)))
			{
				System.out.println("=====(" + DateUtil.getCurTimeStr() + ")=====channel(googleplay)---> Signature verification failed.");
				return false;
			}
			return true;
		}
		catch (Exception e)
		{
			e.printStackTrace();
		}
		return false;
	}

	public static void init()
	{
		params.initParams(GoogleParams.CHANNEL_ID, new GoogleParamsVO());
		
//		params.addApp("default", "default", 									"");
//		params.addApp("galaxy2tw", "com.noumena.android.galaxy2tw", 			"");
//		params.addApp("k6tw", "com.noumena.android.k6tw", 						"");
//		params.addApp("tinywartwgp", "com.nounema.android.tinywartwgp", 		"");
//		params.addApp("zqsdengp", "com.kongzhong.tjmammoth.android.zqsdengp", 	"");
//		params.addApp("zqsdtwgp", "com.kongzhong.tjmammoth.android.zqsdtwgp", 	"");
//		params.addApp("galaxy2korgp", "com.noumena.android.galaxy2korgp", 		"");
//		params.addApp("tinyfortus", "com.noumena.android.tinyfortus", 			"");
//		params.addApp("cqqsl", "com.kongzhong.tjmammoth.android.cqqsl", 		"");
//		params.addApp("cqqslengp", "com.kongzhong.tjmammoth.android.cqqslengp", "");
//		params.addApp("tinywarmol", "com.noumena.android.tinywarmol", 			"");
//		params.addApp("galaxy2twmol", "com.noumena.android.galaxy2twmol", 		"");
//		params.addApp("m1en", "com.noumena.andorid.pocketfort.m1en", 			"");
////		params.addApp("m1en.pro", "com.noumena.andorid.pocketfort.m1en.pro", 	"");
//		params.addApp("m1en.pro", "com.noumena.andorid.pocketfort.m1en.pro", 	"");
//		params.addApp("m1en.na", "com.noumena.andorid.pocketfort.m1en.na", 		"");
//		params.addApp("t6tw", "com.noumena.tjmammoth.android.t6tw", 			"");
//		params.addApp("btw", "com.neocyon.btw", 								"");
//		params.addApp("t6kr", "com.noumena.tjmammoth.android.t6kr", 			"");
//		params.addApp("gaoguaikakao", "com.kongzhong.simlife.sanguokr", 		"");
//		params.addApp("t6us", "com.noumena.tjmammoth.android.t6us", 			"");
//		params.addApp("m1Russia", "eu.inn.pocketfort", 							"");
//		params.addApp("m1Russia", "eu.inn.pocketfort.live", 					"");
//		params.addApp("t6kr", "com.noumena.tjmammoth2.android.t6kr", 			"");
//		params.addApp("test", "eu.kongzhong.pocketfort.test",        			"");
	}
}
